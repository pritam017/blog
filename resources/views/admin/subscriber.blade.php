@extends('layouts.backend.app')@section('title', 'Subscriber')@push('css')
...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
    <div class="card">
        <div class="header">
            <h2>
                ALL SUBSCRIBER ({{ $subscribers->count() }})
            </h2> <br>
                ...
                <table
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $sub->created_at->diffForHumans() }}
                            </td>
                            <td>{{ $sub->updated_at->diffForHumans()}}
                            </td>
                            <td>
                                <button class="btn btn-danger btn-sm" onclick="deleteData({{ $sub->id }})" name="delete" type="button">
                                    <i class="material-icons">delete</i>
                                </button>
                                <form action="{{ route('admin.subscriber.destroy',$sub->id)  }}" id="delete-data-{{ $sub->id }}" method="POST">
                                    @csrf
                                @method('DELETE')
                            </form>
                            </td>
